...
2. 下流のSDK(PyMaiML等)側が、公開されたバージョンに対する範囲指定
   (例: `maiml-domain>=1.0,<2.0`)へ依存を切り替える。`1.0.0`未満の
   バージョンはマイナー`y`の変更が破壊的変更を意味するため、下流側が
   通常のバージョン範囲を指定してもタグ固定と大差ない安定性しか得られ
   ない。`1.0.0`以降はSemVer本来の保証(破壊的変更はメジャー`X`のみ)が
   働くため、メジャーバージョン単位の範囲指定が意味を持つ。

本リポジトリの破壊的変更(`CHANGELOG.md`のバージョニングポリシーに従って
マイナー`y`を上げる変更)は、そのままSDK側にとっても追従が必要な変更に
なるため、GitHub Releaseの作成(通知)は特に重要です。

## 公開APIの範囲(`1.0.0`のSemVer保証対象)

`1.0.0`以降、`CHANGELOG.md`冒頭のバージョニングポリシー(破壊的変更で
メジャー`X`を上げる)が対象とする「公開API」の範囲を、ここで確定させ
ます。

**SemVer保証の対象(公開API)**

- `maiml_domain.__all__`に列挙されている全クラス(XSDの各
  complexType/simpleTypeに1:1対応する、本リポジトリの中心的な成果物)
  について:
  - コンストラクタが受け付けるキーワード引数の名前、必須/任意の別
  - 公開属性の名前と、それが表現するPython型(`str`/`int`/`Decimal`/
    `datetime`/`bytes`など、XSDの値空間に対応する型)
  - コンストラクタが送出する**例外の型**(`ValueError`/`TypeError`)と、
    README.mdの「バリデーションの範囲」節に明記された送出条件

**SemVer保証の対象外**

- `maiml_domain.__all__`に含まれないモジュールレベルの実装詳細。
  たとえば`simple_types.NCNAME_PATTERN`/`LANGUAGE_TAG_PATTERN`/
  `UUID_PATTERN`のような検証用正規表現オブジェクトや、`property.py`の
  `_check_ncname_lexical()`のようなアンダースコア始まりのヘルパー関数
  は、`from maiml_domain.simple_types import *`経由で技術的に参照
  できてしまう場合でも、公開APIとは見なしません。予告なく変更・削除
  されることがあります。
- 例外メッセージの**文言そのもの**(型と送出条件のみが保証対象です)。
- クラスがどのモジュールファイル(`property.py`/`core.py`など)に
  定義されているか、というリポジトリ内部のファイル分割。
- README.mdの「バリデーションの範囲」節で「別Issueとして検討する対象」
  と明記されている項目(`xs:token`のwhiteSpace正規化、`xs:anyURI`の
  到達性検証など)。これらは今後追加されうる**新しい検証**であり、
  追加自体は後方互換の機能追加(パッチ/マイナーバージョンアップ)として
  扱います。

新しいクラス・属性・検証を追加する、または上記一覧にあるクラスの
コンストラクタ引数・属性・例外条件を変更する際は、この一覧とREADME.mdの
「バリデーションの範囲」節をあわせて更新してください。
