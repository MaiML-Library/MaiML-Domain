...
に準拠します。`1.0.0`(2026-09-15)以降は標準的なSemVerに従い、以下の
基準でバージョンを管理します。

- **破壊的変更**(公開APIの後方互換性を壊す変更。対象範囲は
  `CONTRIBUTING.md`の「公開APIの範囲」節を参照): `x`(メジャー)を上げる。
  あわせて**GitHub の Release 機能**でリリースノートを作成する。
  Releaseを作成するとリポジトリをWatchしているメンバーに通知が届くため、
  破壊的変更を能動的に知らせる目的も兼ねる。
- **後方互換を保った機能追加**: `y`(マイナー)を上げる。
- **後方互換を保ったバグ修正**: `z`(パッチ)を上げる。
- マイナー・パッチの場合は軽量な `git tag` のみでよく、GitHub Releaseの
  作成は不要。

なお`1.0.0`に到達するまでの開発初期(`[0.1.0]`〜`[0.3.0]`)は、SemVerの
慣習に従いメジャーバージョン`x`を`0`に固定し、既存の挙動を変える変更でも
マイナー`y`を上げる運用でした。「`1.0.0`へ上げるタイミング(=正式リリース)は、
APIが安定し外部(PyMaiML等のSDK)からの利用に耐えると判断した時点とする」
としていた基準に従い、`1.0.0`をリリースしています。

## [Unreleased]

## [1.0.0] - 2026-09-15

- **正式リリース(`1.0.0`)としてバージョニングポリシーを確定しました。**
  これ以降、`maiml_domain.__all__`に列挙された全クラスのコンストラクタ
  引数・公開属性・送出例外の型がSemVer互換性保証の対象になります
  (詳細はCONTRIBUTING.mdの新設「公開APIの範囲」節を参照)。対象外
  (内部ヘルパー関数・正規表現オブジェクト・例外メッセージの文言など)
  も同節に明記しました。
- **`Development Status`分類子を`3 - Alpha`から`5 - Production/Stable`へ
  更新しました。**
- **README.md/CONTRIBUTING.mdの依存先切替の記載例を`maiml-domain>=1.0,<2.0`
  に更新しました。** `1.0.0`未満はマイナー変更が破壊的変更を意味する
  ため実質的な意味を持たなかったこの記載が、`1.0.0`到達により実際に
  使える運用手順になりました。
- コード変更はありません。`0.3.0`までの検証ロジック・型定義がそのまま
  `1.0.0`の公開APIとして確定します。
